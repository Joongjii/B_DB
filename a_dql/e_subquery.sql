...
#단일행, 단일열 서브쿼리
#비교연산자 사용alter
# select절에서 서브쿼리는 단일행 단일열 서브쿼리만 사용가능

select *
from employee
where dept_code =(select dept_code from employee where emp_name like '노옹철');

select emp_name, (select dept_codefrom employee where emp_name like '노옹철')
from employee;

#다중행 서브쿼리 
#서브쿼리의 조회결과 행이 여러개인 서브쿼리
#비교연산자 + IN, ANY, ALL, EXISTS 연산자와 조합하여 사용


# IN
#다중행일때 쓸 수 있지만 단일열에서도 쓸 수 있다
#최대급여가 4999999보다 적은 급여등급을 가지고있는 직원을 조회
...
# ANY
#서브쿼리의 결과값과 컬럼값간의 비교연산결과가 하나라도 true가 된다면 true
...
# 상관쿼리(상호연관쿼리)
# 메인퀄에서 사용한 컬럼값을 서블쿼리에서도 사용하는 쿼리
# 메인쿼리의 컬럼값이 변경이 되면 서브쿼리의 결과값도 바뀐다

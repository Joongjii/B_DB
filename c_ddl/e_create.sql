...
# delete from our_dept where dept_code = 'D01'; == 삭제 불가
				# -- 자식 테이블이 디고일로 식별되는 행을 참조하고 있다
...
#외래키 추가
# 외래키 제약조건에 대한 옵션 지정
# on delete cascade : 부모 테이블에서 자식테이블이 참조하고 있는 행이 삭제될 경우, 자식 테이블의 행도 함께 삭제
# on delete set null : 부모테이블에서 자식 테이블이 참조하고 있는 행이 삭제될 경우, 자식테이블의 외래키를 null 처리
# on delete restrict : 부모테이블에서 자식 테이블이 참조하고 있는 행을 삭제할 행이 없음(default) 

# on delete cascade : 부모 테이블에서 자식테이블이 참조하고 있는 행이 수정될 경우, 자식 테이블의 행도 함께 삭제
# on delete set null : 부모테이블에서 자식 테이블이 참조하고 있는 행이 수정될 경우, 자식테이블의 외래키를 null 처리
# on delete restrict : 부모테이블에서 자식 테이블이 참조하고 있는 행을 수정할 행이 없음(default) 
alter table our_emp add constraint fk_our_dept
foreign key(dept_code) references our_dept(dept_code) on delete set null;
# 삭제시에 외래키에 null값을 넣는다
